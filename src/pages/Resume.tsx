  ...
  const sections = {
    experience: {
      ...
      content: [
        ...
        },
        {
          ...
          period: "2020 - 2022",
          description: "Разработка интерактивных веб-приложений с использованием React и Vite",
        },
        {
    ...
    skills: {
      ...
      content: [
        ...
        { category: "РАЗРАБОТКА", items: ["React", "Vite", "TypeScript", "Framer Motion"] },
        { category: "ИНСТРУМЕНТЫ", items: ["Git", "Vite", "Sass", "Tailwind CSS"] },
      ],
    },
    education: {
      title: "ОБРАЗОВАНИЕ",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          >
              ...
              <div className="text-center mb-6">
                <div
                  className={`w-32 h-32 mx-auto mb-4 border-4 ${
                    isDark ? "bg-brutal-green border-brutal-white" : "bg-brutal-red border-brutal-black"
                  }`}
                ></div>
            ...
            <div className="space-y-2">
              {Object.keys(sections).map((key) => (
                <motion.button
                  key={key}
                  className={`w-full text-left font-mono p-3 border-2 transition-all ${
                    activeSection === key
                      ? isDark
                        ? "bg-brutal-yellow text-brutal-black border-brutal-yellow"
                        : "bg-brutal-yellow text-brutal-black border-brutal-black"
                      : isDark
                        ? "bg-dark-surface text-dark-text border-brutal-white hover:bg-brutal-pink hover:text-brutal-black"
                        : "bg-brutal-white text-brutal-black border-brutal-black hover:bg-brutal-pink"
                  }`}
          ...
          >
            <div className={isDark ? "brutal-card-dark" : "brutal-card"}>
              <h2 className={`font-anton text-3xl mb-8 ${isDark ? "text-brutal-green" : "text-brutal-red"}`}>
              ...
              {activeSection === "skills" ? (
                <div className="space-y-6">
                  {(sections.skills.content as any[]).map((skillGroup, index) => (
                    ...
                    >
                      <h3 className={`font-anton text-xl mb-3 ${isDark ? "text-brutal-blue" : "text-brutal-blue"}`}>
                        {skillGroup.category}
                      </h3>
                ...
                <div className="space-y-8">
                  {(sections[activeSection as keyof typeof sections].content as any[]).map((item, index) => (
                    <motion.div
                      key={index}
                      className={`border-l-4 pl-6 ${isDark ? "border-brutal-green" : "border-brutal-red"}`}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: index * 0.1 }}
                    >
                      <h3 className="font-anton text-xl mb-2">{item.title}</h3>
                      <p className={`font-mono mb-2 ${isDark ? "text-brutal-blue" : "text-brutal-blue"}`}>
